...
author: "Efrain Ramirez & Megan Redican"
format: 
  html: default
  gfm: default
...
> one uses brackets as means to pluck the variable from the data
while the other just literally uses the function "pluck"
...
> The first line code piped the input (.) and title starships and then mapped them as a double vector.
> The second line of code just uses maps and the dataset with rather than piping into the dataset.
> The thrid line combines both the first and second line of code by mapping the data into a double vector.
...
sw_ships %>% 
  ...
  ggplot(aes(y = reorder(title, desc(episode)), 
             x = prop)) + 
...
> A New Hope has the second highest percentage of Hyperdrive ships.
...
> Its a positive linear relationship between number of star ships and hyper drives. The Phantom Menace has a below average number of hyperdrive ships given its total number of ships. The Force Awakens used hyperdrive more than the Originals.
